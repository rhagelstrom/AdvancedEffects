...
function update()
	...
	if (sDurDice ~= '') then sDuration = sDuration .. sDurDice; end
	if (nDurationMod ~= 0 and sDurDice ~= '') then
		local sSign = '+';
		if (nDurationMod < 0) then sSign = ''; end
	...
	if sDuration ~= '' then
		...
		if (bMultiple) then sDuration = sDuration .. 's'; end
	end
	...
	if (sVis ~= '') then sVis = ' visibility [' .. sVis .. ']'; end
	if (sDuration ~= '') then sDuration = ' for [' .. sDuration .. ']'; end
