...
function update()
	...
	if bSave and Session.IsHost then
		updateSaveEffects()
	end
	...
	if bSusceptiblity and Session.IsHost then
	...
	if bMisc and Session.IsHost then
		updateMiscEffects()
	end
...
local function updateLabelOnlyEffects(node)
	DB.setValue(node, "..effect", "string", DB.getValue(node) or "")
end
...
local function hideNpcEffects(nodeAdvEffect)
	...
	if sVisibility == "" and sEffectString == "" then
		DB.setValue(nodeAdvEffect, "visibility", "string", "hide")
	end
end
