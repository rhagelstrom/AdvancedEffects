...
function onClose()
    ...
    for _, sItemListNodeName in pairs(ItemManager.getInventoryPaths('charsheet')) do
        local sItemList = 'charsheet.*.' .. sItemListNodeName;
        DB.removeHandler(sItemList .. '.*.carried', 'onUpdate', inventoryUpdateItemEffects);
        DB.removeHandler(sItemList .. '.*.isidentified', 'onUpdate', updateItemEffectsForID);
        DB.removeHandler(sItemList .. '.*.effectlist.*.effect', 'onUpdate', updateItemEffectsForEdit);
        DB.removeHandler(sItemList .. '.*.effectlist.*.durdice', 'onUpdate', updateItemEffectsForEdit);
        DB.removeHandler(sItemList .. '.*.effectlist.*.durmod', 'onUpdate', updateItemEffectsForEdit);
        DB.removeHandler(sItemList .. '.*.effectlist.*.name', 'onUpdate', updateItemEffectsForEdit);
        DB.removeHandler(sItemList .. '.*.effectlist.*.durunit', 'onUpdate', updateItemEffectsForEdit);
        DB.removeHandler(sItemList .. '.*.effectlist.*.visibility', 'onUpdate', updateItemEffectsForEdit);
        DB.removeHandler(sItemList .. '.*.effectlist.*.actiononly', 'onUpdate', updateItemEffectsForEdit);
        DB.removeHandler(sItemList .. '.*.effectlist', 'onChildDeleted', removeEffectOnItemEffectDelete);
        DB.removeHandler(sItemList .. '', 'onChildDeleted', updateFromDeletedInventory);
    end
end
function replaceItemEffects(nodeItem)
    local nodeCT = ActorManager.getCTNode(ActorManager.resolveActor(DB.getChild(nodeItem, '...')));
    if not nodeCT or DB.getValue(nodeItem, 'carried') ~= 2 then
        return;
    end
...
function updateItemEffectsForID(node)
    local nodeItem = DB.getParent(node);
    if nodeItem then
...
function updateItemEffectsForEdit(node)
...
function removeEffectOnItemEffectDelete(node)
