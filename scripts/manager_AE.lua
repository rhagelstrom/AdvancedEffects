...
local function sendEffectRemovedMessage(nodeChar, nodeEffect, sLabel, nGMOnly)
    ...
    local msg = ChatManager.createBaseMessage(
                    ActorManager.resolveActor(nodeChar), sUser);
    msg.text = "Advanced Effect ['" .. sLabel .. "'] ";
    msg.text =
        msg.text .. "removed [from " .. DB.getValue(nodeChar, "name", "") .. "]";
    -- HANDLE APPLIED BY SETTING
    local sEffSource = DB.getValue(nodeEffect, "source_name", "");
    if sEffSource and sEffSource ~= "" then
        msg.text = msg.text .. " [by " ..
                       DB.getValue(DB.findNode(sEffSource), "name", "") .. "]";
    end
...
local function sendEffectAddedMessage(nodeCT, rNewEffect, _, nGMOnly)
    ...
    -- Build output message
    local msg = ChatManager.createBaseMessage(ActorManager.resolveActor(nodeCT),
                                              sUser);
    ...
    if rNewEffect.sSource and rNewEffect.sSource ~= "" then
        msg.text = msg.text .. " [by " ..
                       DB.getValue(DB.findNode(rNewEffect.sSource), "name", "") ..
                       "]";
    end
    sendRawMessage(sUser, nGMOnly, msg);
end
...
local function getEffectsByType_new(rActor, sEffectType, aFilter, rFilterActor,
                                    bTargetedOnly) -- luacheck: ignore (cyclomatic complexity)
    local results = {};
...
local function updateCharEffect(nodeCharEffect, nodeEntry)
    ...
    sendEffectAddedMessage(nodeEntry, rEffect, sLabel, nGMOnly,
                           User.getUsername());
...
local function updateItemEffect(nodeItemEffect, sName, nodeChar, sUser,
                                bEquipped, bIdentified)
    ...
    if sLabel and sLabel ~= "" then -- if we have effect string
            ...
            if (nActive ~= 0) then
                ...
                if (sEffSource == sItemSource) then
                    ...
                    if (not bEquipped) then
                        sendEffectRemovedMessage(nodeChar, nodeEffect, sLabel,
                                                 nGMOnly, sUser)
                        nodeEffect.delete();
                        break
                    end -- not equipped
        ...
        if (not bFound and bEquipped) then
            ...
            local bLabelOnly = (DB.getValue(nodeItemEffect, "type", "") ==
                                   "label");
...
function updateItemEffects(nodeItem)
    local nodeChar = ActorManager.getCTNode(
                         ActorManager.resolveActor(nodeItem.getChild("...")));
    ...
    for _, nodeItemEffect in pairs(DB.getChildren(nodeItem, "effectlist")) do
        updateItemEffect(nodeItemEffect, DB.getValue(nodeItem, "name", ""),
                         nodeChar, nil, bEquipped, bIdentified);
    end
end
...
local function hasEffect_new(rActor, sEffect, rTarget, bTargetedOnly,
                             bIgnoreEffectTargets)
    ...
    for _, v in pairs(DB.getChildren(ActorManager.getCTNode(rActor), "effects")) do
        ...
        if ((not AdvancedEffects and nActive ~= 0) or
            (AdvancedEffects and AdvancedEffects.isValidCheckEffect(rActor, v))) then
...
function onInit()
    ...
    OptionsManager.registerOption2("ADND_AE_EDIT", false,
