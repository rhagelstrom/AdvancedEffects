...
--
-- luacheck: globals onInit hasEffectCondition_new getEffectsByType_new hasEffect_new
function onInit()
...
function getEffectsByType_new(rActor, sEffectType, aFilter, rFilterActor, bTargetedOnly) -- luacheck: ignore (cyclomatic complexity)
	local results = {}
	if not rActor then
		return results
	end
...
function hasEffect_new(rActor, sEffect, rTarget, bTargetedOnly, bIgnoreEffectTargets)
